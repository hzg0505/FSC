...
class Match(nn.Module):
    ...
    def forward(self, feat, exemplar):
        ...
        feat = feat.contiguous().view(self.head, self.head_dim, h_f, w_f) # [head,c,h,w]
        exemplar = exemplar.contiguous().view(self.head, self.head_dim, h_p, w_p) # [head,c,h,w]
...
class Model(nn.Module):
    def __init__(self, 
        ...
        self.conv_out = nn.Sequential(
            nn.Conv2d(head_dim*head*2, head_dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(head_dim),
            nn.ReLU(),
            nn.Conv2d(head_dim, head_dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(head_dim),
        ...
        self.regressor = Regressor(in_dim=head_dim,  activation='leaky_relu')
    ...
    def forward(self, image, boxes):
        ...
        for i in range(self.shot):            # 不同样本
            ...
            for exemplars in exemplars_list:  # 不同尺度
                ...
                match_feat = self.match(feat_match, exemplar)   # [head, head_dim, h, w]
                match_feats_scale_list.append(match_feat)
            match_feats = torch.stack(match_feats_scale_list)   # [scales, head, head_dim, h, w]
            feat_match = torch.sum(match_feats, dim=0)          # [head, head_dim, h, w]
            feat_match = self.conv(feat_match)                  # [head, head_dim, h, w]
        feat_match = feat_match.contiguous().view(1, self.head_dim*self.head, 64, 64) # [1,c*head,h,w]
        feat = torch.concat([feat_orig, feat_match])
        feat = feat.contiguous().view(1, self.head_dim*self.head*2, 64, 64) # [1,c*head,h,w]
